...
public class JSonParser
{
      ...
      while (i < paramJSONObject.names().length())
      {
        ...
        else if ((localJSONException1 instanceof JSONArray))
        {
          if (!tmpApiLst.contains(localObject1 + "/[Array]"))
          {
            ...
            modCommon.Log_d(localObject1 + "/[Array]");
          }
        ...
        else if ((localJSONException1 instanceof String))
        {
          if (!tmpApiLst.contains(localObject1 + "/[String]"))
          {
            ...
            modCommon.Log_d(localObject1 + "/[String]");
          }
        }
        else if ((localJSONException1 instanceof Integer))
        {
          if (!tmpApiLst.contains(localObject1 + "/[int]"))
          {
            ...
            modCommon.Log_d(localObject1 + "/[int]");
          }
        }
        else if ((localJSONException1 instanceof Boolean))
        {
          if (!tmpApiLst.contains(localObject1 + "/[bool]"))
          {
            ...
            modCommon.Log_d(localObject1 + "/[bool]");
          }
        }
        else if ((localJSONException1 instanceof Long))
        {
          if (!tmpApiLst.contains(localObject1 + "/[long]"))
          {
            ...
            modCommon.Log_d(localObject1 + "/[long]");
          }
        }
        else if ((localJSONException1 instanceof Double))
        {
          if (!tmpApiLst.contains(localObject1 + "/[double]"))
          {
            ...
            modCommon.Log_d(localObject1 + "/[double]");
          }
        }
        else if (localJSONException1.toString() != "null")
        {
          modCommon.Log_d("OULALA !!! C'est quoi ce truc ? : " + (String)localObject1 + " / " + localJSONException1.toString());
        }
        i += 1;
      }
    }
  ...
  public static String getJSonLocalFile(int paramInt)
    throws Exception
  {
    float f = (float)SystemClock.currentThreadTimeMillis();
    InputStream localInputStream = GamePak.getStream(paramInt);
    modCommon.Log_w("JSON Feed load time : " + ((float)SystemClock.currentThreadTimeMillis() - f) / 1000.0F + " sec");
    return modCommon.generateString(localInputStream);
  }
    ...
    throws Exception
  {
    ...
    modCommon.Log_i("sendJSonRequest :: " + paramString);
    ...
    modCommon.Log_w("JSON Feed load time : " + ((float)SystemClock.currentThreadTimeMillis() - f) / 1000.0F + " sec");
    return modCommon.generateString(paramString);
  }
...
/* Location:              /Users/amaurremi/uw/stringoid/static-dynamic-comparison/apps/bubble blast/com.magmamobile.game.BubbleBlast-23-dex2jar.jar!/com/magmamobile/game/BubbleBlast/utils/json/JSonParser.class
