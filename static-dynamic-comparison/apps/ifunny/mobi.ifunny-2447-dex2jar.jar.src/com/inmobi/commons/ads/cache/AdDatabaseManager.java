...
public class AdDatabaseManager {
    ...
    protected void close() {
        try {
            ...
            Log.internal("[InMobi]-4.5.1", "Failed to close ads db", localException);
        }
    }
    ...
    public AdData getAd(String paramString) {
        try {
            ...
            for (; ; ) {
                Log.internal("[InMobi]-4.5.1", "Failed to get native ads from db", paramString);
                paramString = null;
            }
        } finally {
        }
    ...
    protected int getDBSize() {
        try {
            ...
            Log.internal("[InMobi]-4.5.1", "Failed to get native ads from db", localException);
        }
        return 0;
    }
    ...
    public void insertAd(AdData paramAdData) {
        try {
            ...
            for (; ; ) {
                Log.internal("[InMobi]-4.5.1", "Failed to insert native ads to db", paramAdData);
            }
        } finally {
        }
    }
    ...
    protected void open() {
        try {
            ...
            Log.internal("[InMobi]-4.5.1", "Failed to open ads db", localException);
        }
    }

    public void removeExpiredAds(long paramLong, String paramString) {
        try {
            open();
            long l = System.currentTimeMillis();
            int i = this.b.delete("ad", "timestamp<? AND adtype=?", new String[]{Long.toString(l - 1000L * paramLong), paramString});
            close();
            Log.internal("[InMobi]-4.5.1", "Deleted " + i + " expired ads from cache.");
            return;
        } catch (Exception paramString) {
            Log.internal("[InMobi]-4.5.1", "Failed to remove expired ads from cache", paramString);
        }
    }
...
/* Location:              /Users/amaurremi/uw/stringoid stuff/static vs dynamic/ifunny/mobi.ifunny-2447-dex2jar.jar!/com/inmobi/commons/ads/cache/AdDatabaseManager.class
