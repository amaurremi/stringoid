...
public class AdTrackerUtils {
    public static boolean checkDownloadGoalAdded() {
        if (InternalSDKUtil.getContext() == null) {
            Log.internal("[InMobi]-[AdTracker]-4.5.1", "Application Context NULL cannot checkStatusUpload");
            return false;
        }
    ...
    public static boolean isPermissionGranted(String paramString) {
        try {
            ...
            Log.internal("[InMobi]-[AdTracker]-4.5.1", "Cant check permissions", paramString);
        }
        return false;
    }
    ...
    public static void reportMetric(AdTrackerEventType paramAdTrackerEventType, Goal paramGoal, int paramInt1, long paramLong, int paramInt2, String paramString) {
        try {
            ...
            Log.internal("[InMobi]-[AdTracker]-4.5.1", "Error reporting metric", paramAdTrackerEventType);
            return;
        }
        paramAdTrackerEventType = new JSONObject();
        paramAdTrackerEventType.put("g", paramGoal.getGoalName());
    ...
    public static boolean sendBroadcastMessage(int paramInt) {
        try {
            ...
            Log.internal("[InMobi]-[AdTracker]-4.5.1", "Cant send test broadcast", localException);
        }
        return false;
    }
    ...
    public static void setInternalReferrer(Context paramContext, String paramString) {
        ...
        try {
            Log.internal("[InMobi]-[AdTracker]-4.5.1", "Cannot set Market Referrer..Referrer NULL");
            return;
        } catch (Exception paramContext) {
            Log.internal("[InMobi]-[AdTracker]-4.5.1", "Cannot set referrer", paramContext);
            return;
        }
        ...
        Log.internal("[InMobi]-[AdTracker]-4.5.1", "Saving referrer from broadcast receiver: " + paramString);
        ...
        paramString = FileOperations.getPreferences(paramContext.getApplicationContext(), "IMAdTrackerStatusUpload", "mk-siteid");
        if ((paramString == null) || ("".equals(paramString.trim()))) {
            Log.debug("[InMobi]-[AdTracker]-4.5.1", "Initialization incomplete. Please call InMobi initialize with a valid app Id");
            return;
        }
    ...
    public static void setReferrerFromLogs(Context paramContext, String paramString) {
        ...
        try {
            Log.internal("[InMobi]-[AdTracker]-4.5.1", "Cannot set Market Referrer from logs..Referrer NULL");
            return;
        } catch (Exception paramContext) {
            Log.internal("[InMobi]-[AdTracker]-4.5.1", "Cannot set referrer from logs", paramContext);
        }
        if (0L == FileOperations.getLongPreferences(paramContext.getApplicationContext(), "IMAdTrackerStatusUpload", "t2")) {
            FileOperations.setPreferences(paramContext.getApplicationContext(), "IMAdTrackerStatusUpload", "t2", System.currentTimeMillis());
        }
...
/* Location:              /Users/amaurremi/uw/stringoid stuff/static vs dynamic/ifunny/mobi.ifunny-2447-dex2jar.jar!/com/inmobi/commons/analytics/iat/impl/AdTrackerUtils.class
